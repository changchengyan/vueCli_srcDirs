...
<div id="selectTree">
    <div class="dse-select">
        <div class="dse-input" @click="showSubTree">
            <input type="text" readonly v-model="searchNames" @blur="closeSub">
        ...
        <div class="sub-model" v-show="data_.length&&subTreeFlag">
            ...
            <el-tree :data="data_" :props="defaultProps" accordion @node-click="handleNodeClick">
            </el-tree>
            <!-- <div class="config">
...
export default {
    components: {},
    props: ["waterWorksTree", "defaultCompony","notTop"],
    data() {
        return {
            rotateEle: false,
            subTreeFlag: false,
    ...
    methods: {
        ...
        cancel() {
            ...
            this.rotateEle = false;
            let temp_json = {}
            temp_json.name = this.DSE.tree[0].label;
            temp_json.id = this.DSE.tree[0].id;
            temp_json.type = this.DSE.tree[0].type;
            this.searchNames = temp_json.name;
            this.$emit("showSelectLists", temp_json);
        },
        handleNodeClick(data) {
            let that = this;
            let flag = this.notTop
            if(data.label.indexOf("集团")>-1&&flag===false){
                return
            }
            if(data.size>0){
                return
    ...
    mounted() {
        this.searchNames = this.defaultCompony
    },
    created() {
        let that = this
        that.data_ = this.waterWorksTree;
    }
};
</script>
