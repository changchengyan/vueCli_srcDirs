...
import { getOrganizationTree } from "@/api/common/get_organization";
import { getOrganDictTree } from "@/api/common/get_area";

import { getDictByPCode } from "@/api/common/base_dirct";
...
export default {
  ...
  methods: {
    btns_allowed(arr) {
      ...
      let local_json = dse.allowBtns.monitor;
      let temp_arr = JSON.parse(JSON.stringify(arr));
      for (let i = 0; i < temp_arr.length; i++) {
        if (temp_arr[i] === local_json.surver.action) {
          temp_json.surver.action = true;
        } else if (temp_arr[i] === local_json.baseSet.edit) {
          temp_json.baseSet.edit = true;
        } else if (temp_arr[i] === local_json.baseSet.del) {
          temp_json.baseSet.del = true;
        } else if (temp_arr[i] === local_json.baseSet.add) {
          temp_json.baseSet.add = true;
        }
      }
      return temp_json;
    },
      ...
      getSessionUser()
        .then(res => {
          ...
          if (res.msg == "no user") {
            ...
            if (
              orgId === "402881d162b284680162b3a41362012a" ||
              orgId === "8ae4b2d5645e202701645ec426720013"
            ) {
            ...
            that.get_water_quality();
            that.get_pipe_deep();
            that.get_road_weight();
            that.get_pipe_age();
            that.get_pipe_burst_num();
            that.get_danger()
    ...
    get_road_weight() {
      ...
      formdata.append("pcode", "road_weight");
      getDictByPCode(formdata)
        .then(res => {
          let temp_arr = [];
          ...
          that.$store.commit("set_road_weight", temp_arr);
          dse.roadWeights = temp_arr;
        })
        .catch(error => {
          console.log(error);
        });
    ...
    get_pipe_age() {
      let that = this;
      let formdata = new FormData();
      formdata.append("pcode", "pipe_age");
      getDictByPCode(formdata)
        .then(res => {
          let temp_arr = [];
          let arr = res && res.length ? res : [];
          ...
          that.$store.commit("set_pipe_age", temp_arr);
    ...
    get_pipe_burst_num() {
      let that = this;
      let formdata = new FormData();
      formdata.append("pcode", "pipe_burst_num");
      getDictByPCode(formdata)
        .then(res => {
          let temp_arr = [];
          let arr = res && res.length ? res : [];
          ...
          that.$store.commit("set_pipe_burst_num", temp_arr);
          dse.orderStatus = temp_arr;
        })
        .catch(error => {
          console.log(error);
    ...
    get_pipe_deep() {
      let that = this;
      let formdata = new FormData();
      formdata.append("pcode", "pipe_deep");
      getDictByPCode(formdata)
        .then(res => {
          let temp_arr = [];
          let arr = res && res.length ? res : [];
          ...
          that.$store.commit("set_pipe_deep", temp_arr);
          dse.orderStatus = temp_arr;
        })
        .catch(error => {
          console.log(error);
    ...
    get_water_quality() {
      let that = this;
      let formdata = new FormData();
      formdata.append("pcode", "water_quality");
      getDictByPCode(formdata)
        .then(res => {
          let temp_arr = [];
          let arr = res && res.length ? res : [];
          ...
          that.$store.commit("set_water_quality", temp_arr);
          dse.orderStatus = temp_arr;
        })
        .catch(error => {
          console.log(error);
    ...
    get_danger() {
      ...
      getDictByPCode(formdata)
        .then(res => {
          ...
          that.$store.commit("set_dangerLayer", temp_arr);
    ...
    get_problem_type() {
      ...
      getDictByPCode(formdata)
        .then(res => {
          ...
          that.$store.commit("set_repairType", temp_arr);
    ...
    get_orderStatus() {
      ...
      getDictByPCode(formdata)
        .then(res => {
          ...
          that.$store.commit("set_monitor_orderStatus", temp_arr);
          dse.orderStatus = temp_arr;
        })
        .catch(error => {
          console.log(error);
